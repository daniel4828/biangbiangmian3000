...
2. ...""",
    ...
    #
    # #1012：第 17 条要求专有名词写成中文译名（原来第 16 条的"原样保留"让
    # Musk/Britain 这类拉丁拼写直接进了中文句子）。这不只是好看：听力提示的
    # 遮挡（#1006）和生词判定（zh_annotate）都是按词判断的，拉丁拼写在两边
    # 都落不到判定里，于是永远显示、也永远进不了生词表。译成中文之后它就是
    # 一个普通词——没学过就当生词显示，已收藏/已标已知就跟着一起被遮住。
...
16. 【关键词例外】素材中的人名、地名、机构名、数字、年份、金额和核心术语，
    即使超出 HSK 等级也必须保留，这些正是最值得记住的内容。
    绝不许简化成「一个人」「一个地方」「很多」「好几年」这类模糊说法。
    关键词数量不设上限——只要句子读得通，一句里出现三四个数字或名字也很好。
17. 【专有名词写成中文】人名、地名、机构名、公司名一律用通行的简体中文译名
    （Musk→马斯克，Britain→英国，Deutsche Bahn→德国铁路），
    绝不许把外语拼写原样搬进中文句子。
    只有确实没有通行中文译名时才允许保留外语原文。
18. 如果某个事实除了关键词以外还需要难词才能说清楚，
    就换一种更简单的说法，实在不行才放弃这个事实。
19. 只用简体字和中文标点。禁止繁体字、markdown、拼音和括号注释。
20. 不要给目标词加引号、括号或任何标记，直接写进句子里。

四、reasoning_zh 的写法（#931：分两段）
21. 每句都要写 reasoning_zh，分两段：
    先写「事实：」加上该句复述的那条事实，照抄其中的名字和数字
    （用素材原文的语言写这部分）；
    再写「为什么：」加一句中文，说明为什么选这条事实——
    它在整篇素材里的分量、回答了什么问题、和前后内容的关系。
    面向 HSK 4-5 学习者，中文部分用词不要太难。
22. 把事实写出来正是为了自查：任何两句的「事实：」都不许相同。
