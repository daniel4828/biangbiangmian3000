...
Turns a knowledge-base episode's summary or full text into an mp3 + a
sentence-level cue list the reading screen can highlight against, using
whichever of audio.build_track()'s alignment paths applies — today only the
text-only TTS path (audio/tts_track.py) is implemented; `owner_kind='book_page'`
...
def _resolve_text(owner_kind: str, owner_id: int, lang: str, variant: str) -> str:
    ...
    raise HTTPException(
        status_code=400,
        detail="book_page audio tracks are not implemented yet (phase 3 of #1047)")
