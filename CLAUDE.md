...
> 🔴 **语言规则（中文回答、含中文的消息先改写+纠正、回答要短）在全局 `~/.claude/CLAUDE.md`，每个项目共用**（#1200）。本文件只放项目专属规则；和全局不同的语言调整写在下方「本地语言调整」节。
...
**工作方式（每次会话都要遵守）**
1. [固定规则](#固定规则mandatory-rules) · 2. [本地语言调整](#本地语言调整全局规则见-claudeclaudemd) · 3. [代理编排：Opus 编排，Sonnet 执行](#代理编排opus-编排sonnet-执行) · 4. [Git & GitHub 工作流](#git--github-工作流) · 5. [语言错误日志](#语言错误日志1129--1200-全局化)
...
| R4 | **一律用中文；含中文的消息先改写他的输入** | 见全局 `~/.claude/CLAUDE.md`（#1200 起不在这里重复）；Issue/PR/提交信息同样用中文 |
...
| R10 | **永远不要使用日语** | 见全局 `~/.claude/CLAUDE.md` |
...
| R15 | **回答又短又简单** | 见全局 `~/.claude/CLAUDE.md`（2026-08-14 起） |
...
## 本地语言调整（全局规则见 `~/.claude/CLAUDE.md`）

这里只写和全局语言规则**不同**的地方，不复制全局规则。
...
- **ASR 专有名词校正（#1215）**：新生成的 NotebookLM/听悟/Whisper/Groq Whisper 转录在入库后续处理前先经过一次 DeepSeek 校正。模型只返回 `original`→`corrected` 的 JSON 替换表，程序精确替换，绝不接受整篇重写；提示词带素材标题和来源名。不确定、无效 JSON、API 异常都原样回退，不能阻断摘要；文章、粘贴正文、官方 YouTube 字幕和 `DISABLE_AI` 模式不调用。
