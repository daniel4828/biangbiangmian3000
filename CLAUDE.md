...
| R4 | **一律用中文回答** | 无论 Daniel 用中文、法语、德语、英语还是混合写，回答全程中文（2026-09-11 起，由 Daniel 决定；此前法语消息用法语回答）。见"语言指令" |
...
| R11 | **每条消息开头必须改写 Daniel 的输入** | 把他的消息改写为正确的**中文**版本，画分割线，再回答。这是帮助 Daniel 学中文的核心机制，绝不可跳过 |
...
| R16 | **不确定才问，有推荐就直接做** | （2026-08-29 起，Daniel 决定）如果我心里已经有推荐方案，就**直接执行**，不要拿选项来问他——他每次都会选推荐的那个，问一遍只是多一轮往返。只有真正不确定、且不同选择会导致**明显不同的结果**时才问。问的时候直接说明取舍，不要列一堆选项。做完之后告诉他做了什么、以及怎么改回来 |
...
- **靠 hook，不靠我记得**：`.claude/settings.json` 的 `UserPromptSubmit` hook 调 `scripts/log_language_errors.sh`。让主会话每条消息自己写文件，既占注意力又一定会漏（R11 已经是一条"绝不可跳过"的规则，再加一条只会更糟），而这恰恰是 Daniel 担心影响回答质量的地方
- **纠正由一次便宜的 Haiku 调用生成**，`async: true` 跑在后台，主回答完全不参与。R11 的行内纠正照旧，两者互不影响
