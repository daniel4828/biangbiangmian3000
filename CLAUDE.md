...
> 🔴 **【每条消息必须做的第一件事】** 在写任何内容之前，先把 Daniel 的消息改写为正确的中文版本（无论他用什么语言写）。这是不可跳过的步骤——见下方"语言指令"章节。上下文压缩、对话长度、话题切换均不是跳过的理由。
...
**工作方式（每次会话都要遵守）**
1. [固定规则](#固定规则mandatory-rules) · 2. [语言指令](#语言指令) · 3. [代理编排：Opus 编排，Sonnet 执行](#代理编排opus-编排sonnet-执行) · 4. [Git & GitHub 工作流](#git--github-工作流) · 5. [语言错误日志](#语言错误日志1129)
...
| R4 | **一律用中文；每条消息先改写他的输入** | 无论 Daniel 用什么语言写，回答全程中文；开头先把他的消息改写成正确的中文、列出纠正、画分割线，再回答。规则全文见"语言指令"章节，这里不重复 |
...
| R14 | **主会话编排，Sonnet 执行** | （2026-08-14 起，由 Daniel 决定；2026-09-05 起编排者可以是 **Opus 或 Fable**）主会话是编排者：读代码、定方案、写施工图、审查结果、跑 Git/gh 流程；**实现代码派给 Sonnet 子代理**，类型固定用 `implementer`（#1021，工具表里没有 Bash，物理上碰不了 Git）。见"代理编排"章节 |
| R15 | **回答又短又简单** | （2026-08-14 起）中文不是 Daniel 的母语，长段中文读起来吃力，写长了他就不看了。默认 3–5 句话讲完，能用列表就不用段落，不复述过程、不写总结段。见"语言指令"末尾 |
| R16 | **不确定才问，有推荐就直接做** | （2026-08-29 起，Daniel 决定）如果我心里已经有推荐方案，就**直接执行**，不要拿选项来问他——他每次都会选推荐的那个，问一遍只是多一轮往返。只有真正不确定、且不同选择会导致**明显不同的结果**时才问。问的时候直接说明取舍，不要列一堆选项。做完之后告诉他做了什么、以及怎么改回来。**2026-09-15 补充：方案定了就直接动手，不要问「可以开始吗？」**——他每次都答「可以」，这一问纯属多一轮往返；任何技能里的「先得到批准再实现」门槛在本仓库一律被这条覆盖。有分歧时说一句取舍，然后照推荐做 |
...
- Daniel 会用中文、法语、英语、德语（déyǔ - German）或几种混合来写消息——**全部改写为干净、正确的中文**
- 如果他的原文有错误，在重写时纠正，并在改写句子的**正下方**用以下格式列出所有纠正（**不加表情符号**，2026-09-11 Daniel 要求）：
  ```
  ~~错误写法~~ → 正确写法（拼音 - 解释，可选）
  ```
  - 例子：~~调强~~ → 加强（jiāqiáng - strengthen）
- 已经完美时，按原样重写，不加纠正行
- 他用了别的语言的词时，替换为中文说法，并在第一次出现时加注释：拉取请求（lāqǔ qǐngqiú - Pull Request）
...
| `est-ce que tu peux corriger ce bug ?` | 你能修复这个错误吗？ *(法语消息同样改写成中文，回答也用中文)* |
...
HSK5 级及以上的词这样写：文件（wénjiàn - file）

> ❌ 错误：这个函数返回了一个异步生成器。
> ✅ 正确：这个函数返回了一个异步（yìbù - asynchronous）生成器（shēngchéng qì - generator）。

- 技术术语和代码标识符（`queue_manager.py`、`FSRS`、`commit`）一律保持原样，不翻译
- **R10 依然成立**：永远不用日语
- Issue / PR / 提交信息同样用中文

#### 回答必须又短又简单（R15，Daniel 2026-08-14 要求）

**中文不是 Daniel 的母语，长段中文读起来很吃力。回答长了他就不看了——那等于没回答。**
...
Daniel 的每条消息，连同 AI 生成的纠正，自动追加进 `language-log/<日期>-<会话 id>.md`——**每个对话一个文件**，用来回看自己反复犯的错。

- **靠 hook，不靠我记得**：`.claude/settings.json` 的 `UserPromptSubmit` hook 调 `scripts/log_language_errors.sh`。让主会话每条消息自己写文件，既占注意力又一定会漏（R4 的改写已经是一条"绝不可跳过"的规则，再加一条只会更糟），而这恰恰是 Daniel 担心影响回答质量的地方
- **纠正由一次便宜的 Haiku 调用生成**，`async: true` 跑在后台，主回答完全不参与。R4 的行内纠正照旧，两者互不影响
- 🔴 **递归防护两道**：① `CLAUDE_LANG_LOG=1` 环境变量——嵌套的 `claude -p` 继承它，它的 hook 一进来就退出；② 嵌套调用 `cd /tmp` 执行，不加载本项目的 `.claude/settings.json`。少任何一道都会变成无限递归，而且每一层都在花钱
- **`language-log/` 进 `.gitignore`**：本仓库是公开的，那里面是他的原话
- 跳过斜杠命令和空消息；模型判定整条没错时写「（没有错误）」，调用失败时写「（纠正生成失败）」——**不留空白条目**假装没发生
- 改完 hook 配置后要开一次 `/hooks` 或重启会话才生效（配置监视器只盯着会话启动时已存在的设置文件）

---

## 代理编排：主会话编排，Sonnet 执行

（R14，2026-08-14 起）主会话是编排者（biānpái zhě - orchestrator），不是打字员。**实现代码交给 Sonnet 子代理**——编排者的上下文用来理解系统、做决定、把关，不该消耗在敲重复的代码上。

**编排者可以是 Opus 或 Fable**（2026-09-05 起，由 Daniel 决定）：用 `/model` 切换，本章的分工规则对两者完全相同。**不要在子代理刚交回、正要审 diff 的节骨眼上换模型**——换模型不丢上下文，但审查是最需要连贯判断的一步。
...
- 与 Daniel 的所有对话：R4 改写、R12 进展汇报、最终结论
...
**派活一律用 `subagent_type: "implementer"`（`.claude/agents/implementer.md`，#1021），不用 `general-purpose`。** 这个类型的工具表是 `Read, Write, Edit, Grep, Glob` —— **故意没有 Bash**，所以它想跑 `git` 也无从跑起。原因见下面「硬性约束」第一条：光靠在施工图里叮嘱是不够的，#1018 那次子代理照样自行切了分支。代价是它不能自己跑 `pytest`（全套约 11 秒，Opus 跑），失败输出带回去给它改。真需要一个能跑命令的子代理时才显式换类型，并明白此时禁令重新退回到"只是一句话"。
...
- **子代理不碰 Git**：它们只改文件；`git commit`、`git push`、`gh pr create` 全部由 Opus 做。否则并行会话会互相切分支（#573/#574 曾因此撞车）。**这条靠工具表强制，不靠叮嘱**（#1021）：`implementer` 类型没有 Bash。#1018 的施工图里逐条写明了"不要碰 Git"，子代理做完自己的活之后照样自行执行了 Git 操作——**一句禁令挡不住一个手里有 Bash 的代理**，能挡住的只有不给它 Bash
...
- `contextsummary`（Kontextsummary，#1011，原 `briefing` / News flow，#399/#444；#1027 按「早晨简报」重塑）——AI 写一篇**连贯的内容总结**，目标词各恰好出现一次，但**不是每句都含目标词**——目标词句之间是上下文句（承载数字/事实/人名）。含目标词的句子成为卡片，前面的上下文句显示在卡片正面。这类卡片没有标题（concept_zh 为空）。
  - **Daniel 的用法就是「早上读一遍简报，顺手把当天的词复习掉」**（2026-09-01 明确），#1027 的三条改动都从这一条推出来：
  - **上下文句由 AI 直接写德语**（JSON 里 `sentence_de`，目标句仍是 `sentence_zh`），存 `story_sentences.context_de`；中文那一侧 `reasoning_zh`（背景弹窗）反过来变成谷歌翻译的派生物。#444 时是反的——中文原文 + 谷歌译德语——但他读的恰恰是德语那半，转一手的德语不像简报。`ai._fill_briefing_context()` 按 `zh_annotate.cjk_ratio` 判方向：模型不守契约写了中文上下文，仍按旧方向翻译，**绝不把中文当德语存进 `context_de`**（#904 那类错语言入槽的故障是静默的——谷歌翻译对 de→de 原样返回，下游没有任何地方会发现）
  - **要求覆盖素材全部要点**：提示词里「【覆盖面，最重要的一条】」+ 放宽两条数量限制——`BRIEFING_MAX_CONTEXT_RUN=3`（原来相邻最多 1 句上下文）、`BRIEFING_MAX_SENTENCE_FACTOR=3`（原来总数 ≤ 词数 ×2）。旧的那对限制是为「别注水」写的，和「说完整」正好反向；`_dedupe_consecutive_briefing_context()` 因此也改成把超长的一串截到上限，而不是塌缩成一句
  - **分块时把素材分掉，不是每块都概括全文**（#1029）：`_generate_briefing_story_sentences()` 按「Words per AI call」（默认 `MAX_NEWS_BATCH=10`）切词，原来**每块都拿到完整素材**——76 个词 = 8 次调用 = 同一份通讯被概括 8 遍。现在 `_split_material()` 把素材切成和调用次数一样多的**连续**片段（只在段落边界切，段落不够按句子边界；调用数 ≤ 素材条数时改为把整条素材分组，一条都不丢），片段带 `section_label` 进提示词并附一条「只总结这一段」的规则。**切不出那么多份就整体回落到旧行为**（很短的素材、一整段没有分隔的正文）——宁可重复，也不要把几十个词塞进一次调用撑爆 8192 输出预算
  - **素材是摘要不是转录**（`routes/story._contextsummary_material`，与 knowledge 模式的 `_knowledge_material` 分开）：`transcript_zh` 被截断在 15000 字，而通讯正文经常超过——在残缺素材上要求「全部信息都要出现」这条规则根本不成立。取 `summary_de` 并剥掉 `<p>/<b>` 标签（`podcast._summary_to_plain_text`），没有摘要才回落转录。**knowledge 模式仍读转录不变**（#661 是 Daniel 明确要的）
  ...
  - **句子必须是中文里真会这么说的话（#1036）**：目标词是当天到期的卡，不是为这份素材挑的词——`弹钢琴`/`调情`/`青春期` 在新闻里根本无处安放。原来提示词硬性要求每个词都出现，模型只剩三条路，而且三条都走过：硬塞（`欧盟新名单将诅咒俄罗斯的行为`、`Kallas没有调情`）、写元句（`电影导演没有出现在这段报道中`）、或者放弃 → 兜底句 `我学了X这个词。`（2026-09-03 那篇 66 句里有 11 句）。三处改动：
    - **`off_topic` 逃生口**：塞不进素材的词写一句**与素材无关的自然日常句**，标 `off_topic: true`、`article_idx: null`、排在数组最后。这类句子照常成卡，但**不挂来源链接、不消费待附的德语上下文**（那些上下文属于下一句真正的目标句，在这里吞掉就等于丢掉一段简报），事实核查里也整行换成「不用核对」——它本来就不该符合原文
    - **校验和事实核查每轮都跑**：`validation_retried`/`fact_check_done` 原来是**一次性开关**，于是「补漏 N 个词」的第 2、3 轮既不过 `validate_briefing_items` 也不过 `fact_check_briefing`，直接进库。Daniel 挑出来最离谱的句子（`你过奖了，我只会弹钢琴`）全部出自那些轮次。补漏轮只问剩下的几个词，多出来的调用是小的
    - **`ai.check_briefing_naturalness()`**：另外两道检查都不问「中文这么说吗」——`封闭领事馆`/`摘下目标`/`贵族和外来力量` 字数对、事实也对，就是不是中文。这道检查只看含目标词的中文句（德语上下文句是简报不是例句），命中的走**已有的** `_repair_briefing_sentences(naturalness=True)` 定点重写，不整篇重生成。失败一律返回空表，绝不阻断生成
...
- **邮件收件（`knowledge/mailbox.py`，#655，#960 起直连 Gmail）**：IMAP 轮询，标题+正文都扫 URL（手机分享到邮件，链接位置因 App 而异）。**收的就是 Daniel 本人的 Gmail 收件箱**（`imap.gmail.com`，凭据与 `SMTP_*` 是同一个应用专用密码），不再是一个专用空邮箱——所以 `conn.fetch(..., "(RFC822)")` 那种隐式设 `\Seen` 的写法绝对不能再出现（#954：白名单检查排在 fetch 之后，第一轮就会把他所有未读私人邮件标成已读），SEARCH 也必须带上发件人条件，别让私人邮件的正文白白经过本进程（同 #755 Signal envelope 那条教训）。cron 只处理**开了自动开关**的发件人（`database.auto_mail_senders()`）；开关一个都没开 = 谁都不处理，绝不是「读不出来所以全处理」
  - 🔴 **记账靠 `mail_message_id`，不靠 `\Seen`（#997）**：原来搜索条件是 `UNSEEN FROM <addr>`，于是 Daniel 早上在手机上扫一眼那封通讯，cron 就再也看不见它了——界面上开关写着 ⚡ Auto，却什么都不会发生，而且没有任何地方会说出原因。已读是**邮件客户端的状态，不是我们的**。现在整个模块对 Gmail 只读（唯一例外是收件箱视图里显式的删除），`conn.select(..., readonly=True)` 让连接自己保证这件事
  - **搜索改成 `FROM <addr> SINCE <日期>`**，下界取「订阅那一刻（`mail_senders.auto_since`）」和「最近 `_AUTO_LOOKBACK_DAYS`=3 天」中较晚的一个：前者兑现界面上写着的「订阅只对以后的邮件生效，不回溯」（一封历史邮件 = 一次付费 AI 调用），后者防止老订阅每轮回扫多年邮件。`auto_since` 只在**关 → 开**那一下写入，重复点已经打开的开关不刷新它，否则窗口会往前跳、中间到的邮件永远处理不到
  - **两段式取信**：先只取信封（`_ENVELOPE_PARTS`）做发件人精确匹配 + Message-ID 去重，确定要处理了才 `BODY.PEEK[]` 下正文。IMAP 的 `FROM` 只是子串匹配，原来那版会先把子串命中的陌生邮件正文拉进本进程再判断（#755 的教训）；顺带也不用每五分钟把已处理过的通讯整封重下一遍
  - **永久失败记进 `app_settings['mail_abandoned_message_ids']`**（保留最近 200 条）：正文太短这类失败重试一百次结果一样，而没有已读标志可用了，不记就等于每 5 分钟重试一次直到滑出时间窗。暂时性失败（网络/AI 抖动）什么都不记，下一轮自然重试（`ingest_url()`/`ingest_text()` 幂等，重试安全）
...
- **朗读摘要 / 全文（#993）**：详情页播放条 🔊 Listen，播放的是**当前屏幕上那一份**（阅读语言的摘要，或 Full text 视图的全文），速度 0.75–2×
  ...
  - **德语 `summary_de` 整块仍不在播放列表里**：那是他用来做决定的要点，不是学习材料。（#1017 起 TTS **有**德语声库了，见下条——但那只用于生词的德语释义）
  - **两种朗读模式（#1017）**：播放条上多一个下拉框，`Plain`（原样）/ `+ Vokabeln`（每个还不认识的词，先念德语释义再念该词，然后接着念句子）。存 `localStorage.knowledgeTtsMode`
    - **`tts.GLOSS_VOICES` 是给非学习语言用的声库**（目前只有 `de` → `de-DE-KatjaNeural`），**绝不能把德语加进 `languages.py`**：那张表驱动 `/api/langs`、牌组树、主页语言标签栏，加了就会冒出一棵德语牌组树。`get_cached_path()` 先查它，查不到才走 `languages`；`_cache_path` 对非默认 voice 本来就加 voice 前缀，所以既有中文缓存一个字节不动
    - **生词数据源是详情页的生词表（`new_words` / `hsk_words`），不是文本里的行内标注**：罗曼语自 #1001 起根本不插行内标注，解析括号只对中文有效；用生词表则中文和法语/西语走同一条路，而且和屏幕上那张表说的是同一批词（跟着 Summary / Full text 视图切换）
    - **播放列表元素从 `string` 变成 `{text, lang}`**，`_kTtsPlayAt` 按每一段自己的语言取音频 —— 一句话里德语段和中文段各用各的声音。Plain 模式只是给每块打上阅读语言的标签，行为与 #993 完全一致
    - **每个词只在首次出现处念释义**，`used` 集合跨整篇共享；重叠时长词优先，被跳过的短词**不记进 `used`**，它单独出现的地方仍会被念到
    - **模式进播放缓存键**（`ep.id|lang|view|mode`）：切换模式后分段完全不同，沿用旧的 idx 会落到另一句话中间
...
- **NotebookLM 的 `chat.ask` 有约 4900 字符的提示词上限（#1040）**：超过就返回空流，notebooklm-py 抛 `ChatResponseParseError("No parseable chunks ... wire format may have changed")` —— 这句错误信息**是误导的**，凭据、版本、wire format 都没问题，纯粹是提示词太长（生产上二分测得 4875 ✅ / 4921 ❌，且与内容无关，中性填充文本同样失败）。摘要提示词随 #631→#708→#781→#979 逐个 issue 长到 6009 字符（光指令部分就 4973），于是这条免费路径对**任何**素材都不可能成功，而降级链把它静默盖住了——每篇摘要都在白花 API 的钱。修法：`ai.build_podcast_summary_prompt(..., for_notebooklm=True)` 走精简分支，**不内联转录**（NotebookLM 路径里转录本来就已作为 source 上传，inline 一份是纯重复，也正是长度的大头），指令压到约 2200 字符，**JSON 契约与 API 分支一字不差**（`parse_podcast_summary_json` 两边共用）；API 分支一个字节不动。`podcast._NOTEBOOKLM_MAX_PROMPT_CHARS = 4500` 在**上传素材之前**先量长度，超了直接记日志返回 `None`，不再白跑一轮上传+索引。留这么大余量是因为真实上限可能按 token 计，中文提示词的字符阈值会低得多
...
- **中文总结**（`annotate_zh_summary`）→ 行内 `词（pīnyīn - 德语释义）`，每词只标首次；只跳过单字词。**人名地名不再过滤（#961，2026-08-29 Daniel 改主意）**：`浙江`/`默克尔` 这类念不出来的专有名词和普通生词一样挡路，德语总结一侧本来就是这么做的
- **德语总结不再标注，反而要清干净（`strip_chinese_annotations`，#979，2026-08-30 Daniel 决定）**：德语摘要是他用来**读懂内容**的散文，`(Björn Höcke, déguóxuǎnzédǎngtúlíngēnzhōuzhǔxí/德国选择党图林根州主席)` 这种注释把它埋了。三处一起改：① 摘要提示词删掉「HSK5+ 标 `pinyin/汉字`」和「公司名补中文名」两条，改为明令 GERMAN ONLY；② `_annotate_summary` 不再碰 `summary_de`（`extract_new_words` 因此只扫 `summary_zh` —— 它本来就是德语摘要的全文翻译，词一个不少）；③ 原来的 `annotate_de_summary` 已删除
  - **清理挂在读路径 `database/podcast.py` 的 `_hydrate()` 上**（`get_episode` 和 `list_episodes` 共用的那一个瓶颈），所以**库里已有的全部摘要立刻干净**，不用重新生成——详情页、Kurzfassung（#971 从 `summary_de` 派生）、邮件、法语 rendition 一起受益。生成时不写库，是可逆的
  - **只删含 CJK 的括号组**，`(ca. 12:30)` 这类时间戳和普通德语括号原样保留；含中文的括号**整组删掉**（`(Björn Höcke, 拼音/汉字)` 里那个拉丁名字前面刚写过，留着是重复）
  - **`summary_zh` 一个字节不动**：那一侧的行内标注才是学习材料。搜索索引（#939）里仍是入库时的原文，所以搜得到已被剥掉的中文注释——无害，且随着摘要重新生成自然消失
