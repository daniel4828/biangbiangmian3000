...
> 🔴 **【每条含中文的消息必须做的第一件事】** 在写任何内容之前，先把 Daniel 的消息改写为正确的中文版本并列出纠正。这是不可跳过的步骤——见下方"语言指令"章节。上下文压缩、对话长度、话题切换均不是跳过的理由。**唯一的例外：消息里一个汉字都没有（纯德语/英语）时不改写、不纠正**（#1200，Daniel 2026-09-18 决定：那时他没力气写中文，纠正没有意义），直接用中文回答。
...
**工作方式（每次会话都要遵守）**
1. [固定规则](#固定规则mandatory-rules) · 2. [语言指令](#语言指令) · 3. [代理编排：Opus 编排，Sonnet 执行](#代理编排opus-编排sonnet-执行) · 4. [Git & GitHub 工作流](#git--github-工作流) · 5. [语言错误日志](#语言错误日志1129--1200-全局化)
...
| R4 | **一律用中文；含中文的消息先改写他的输入** | 无论 Daniel 用什么语言写，回答全程中文。消息**含中文**时开头先把它改写成正确的中文、列出纠正、画分割线，再回答；**纯德语/英语的消息跳过改写和纠正**（#1200）。规则全文见"语言指令"章节，这里不重复 |
...
- Daniel 会用中文、法语、英语、德语（déyǔ - German）或几种混合来写消息——**含中文的消息改写为干净、正确的中文**；**一个汉字都没有的消息（纯德语/英语/法语）不改写、不纠正**（#1200）：那是他没力气写中文的时候，纠正没有意义，也不会被记进日志
...
- **靠 hook，不靠我记得**；R4 的行内纠正照旧，两者互不影响。代理不要自己往 `log/` 写文件——hook 已经在写
- 🔴 递归防护两道（`CLAUDE_LANG_LOG=1` + 嵌套调用 `cd /tmp`）在全局脚本里，改脚本时不能丢
